...
M.correct(=(Id, Exp)) := M.expect_type(VarInfo.type, Exp, _) :-
...
M.correct(while(While, Do)) := M.expect_type(boolean, While, _).epush().correct(Do).epop().put(returned, M.returned).
...
M.correct(if(If, Then, Else)) := M.expect_type(boolean, If, _).merge_return(
...
M.correct(decl(Type, Decls)) := M.decls_correct(Type, Decls).

M.decls_correct(_, []) := M.
M.decls_correct(Type, [H|T]) := M.decl_correct(Type, H).decls_correct(Type, T).

M.decl_correct(Type, init(Id, Exp)) := M.expect_type(Type, Exp, _).decl_correct(Type, noinit(Id)).
